...
{
  internal sealed class ItemContainerRecyclingPool : IEnumerable<DependencyObject>
  {
    internal int Count
    {
      get
    ...
    internal void Enqueue( object hint, DependencyObject container )
    {
      if( container == null )
        throw new ArgumentNullException( "container" );

      var key = ( hint != null ) ? hint.GetHashCode() : 0;
      var entry = default( Entry );

      if( !m_collection.TryGetValue( key, out entry ) )
      {
        entry = new Entry();
      }
      ...
      m_collection[ key ] = entry;
      m_count++;
      m_version++;
    }

    internal DependencyObject Dequeue( object hint )
    {
    ...
    private readonly Dictionary<int, Entry> m_collection = new Dictionary<int, Entry>();
