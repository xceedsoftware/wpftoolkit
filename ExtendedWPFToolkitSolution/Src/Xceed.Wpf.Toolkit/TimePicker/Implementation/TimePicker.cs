    ...
    public class TimePicker : DateTimePickerBase
    {
        ...
        public IEnumerable GenerateTimeListItemsSource()
        {
            ...
            if (this.Value.HasValue)
            {
                ...
                if (date.Date == minDate.Date && time.Value.Ticks < minTime.Ticks)
                {
                    time = minTime;
                }

                if (date.Date == maxDate.Date && endTime.Value.Ticks > maxTime.Ticks)
                {
                    endTime = maxTime;
                }
            ...
            if (time != null && endTime != null && timeInterval != null && timeInterval.Value.Ticks > 0)
            {
                while (time <= endTime)
                {
                    timeItemList.Add(this.CreateTimeItem(time.Value));
                    time = time.Value.Add(timeInterval.Value);
                }
            }
            return timeItemList;
        }
        ...
        protected virtual TimeItem CreateTimeItem(TimeSpan time)
        {
            ...
            string formatString = this.GetFormatString(Format);
